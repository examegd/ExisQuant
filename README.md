...
* `AWQpacker(only Q4).py` - python encoder for the hybrid 3-bit/4-bit awq compilation
* `AWQunpacker(only Q4).py` - python decoder for the hybrid 3-bit/4-bit awq compilation!!!!!!!!!
* `packer.py` - weight-only vector quantization compiler (4d k-means)
* `unpacker.py` - A decompressor that converts the model back to the GGUF format from the binary .exiq format
